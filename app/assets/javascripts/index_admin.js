  ...
  for (var i = data.sum_of_row - 1; i >= 0; i--) {
    ...
    for (var j = data.sum_of_collum - 1; j >= 0; j--) {
      for (var k = 0; k < data.seats.length; k++) {
        if(parseInt(data.seats[k].num_of_row) == i && (data.seats[k].num_of_collum - 1) == j){
          if(data.seats[k].status == true){
            if (data.seats[k].seat_type_id == 1) {
            ...
            if (data.seats[k].seat_type_id == 2) {
